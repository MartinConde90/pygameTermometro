...
class Termometro():
    def __init__(self):
        self.custome = pygame.image.load("images/termo1.jpg")
        
class NumberInput():
    __value = 0
    __strValue = '0' # se crea porque "__value = 0" no es un string
    __position = [0, 0]
    __size = [0, 0]
    
    def __init__(self, value=0):
        self.__font = pygame.font.SysFont('Arial', 24)
        self.value(value)# se llama a sí mismo, porque aqui hace lo mismo que en el try de abajo, sería repetirse
        
        '''
        try:
            self.__value = int(value) # si es un numero, continua
            self.__strValue = str(value) #transforma el numero en string
        except:
            pass
        '''
    ...
    def on_event(self, event):
        if event.type == KEYDOWN:
            if event.unicode.isdigit() and len(self.__strValue) < 10:  # o if event.unicode in '0123456789':
    ...
    def render(self):
        textBlock = self.__font.render(self.__strValue, True, (74, 74, 74)) # renderiza el texto a meter en el recuadro
        rect = textBlock.get_rect() # crea el rectangulo y le damos la posicion y tamaño a continuacion
...
class mainApp():
    ...
    def __init__(self):
        ...
        self.__screen.fill((244, 236, 203)) #color de fondo
